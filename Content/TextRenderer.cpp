...
void TextRenderer::Render(const std::wstring& str) {
	...
	m_whiteBrush->SetColor(D2D1::ColorF(D2D1::ColorF::AliceBlue));
...
void TextRenderer::RenderTextOffscreen(const std::wstring& str) {
	...
	D2D1::Matrix3x2F screenTranslation = D2D1::Matrix3x2F::Translation(
		m_textureWidth * 1.0f,
		m_textureHeight * 1.0f + metrics.height * 1.0f
	);
	m_whiteBrush->SetTransform(screenTranslation);
...
concurrency::task<void> TextRenderer::CreateDeviceDependentResourcesAsync() {
	return concurrency::create_task([this] {
		...
		DX::ThrowIfFailed(m_d2dRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::AliceBlue), &m_whiteBrush));
		...
		DX::ThrowIfFailed(m_textFormat->SetParagraphAlignment(DWRITE_PARAGRAPH_ALIGNMENT_FAR));
	});
}
