...
void HolographicFaceTrackerMain::ProcessFaces(std::vector<BitmapBounds> const& faces, Windows::Media::Capture::Frames::MediaFrameReference^ frame, SpatialCoordinateSystem^ worldCoordSystem) {
	...
	float3 const textOffsetInWorldSpace = float3{ 0.0f, 0.25f, 0.0f };
	...
	m_textRenderer->SetTargetPosition(bestRectPositionInWorldspace + textOffsetInWorldSpace);
...
HolographicFrame^ HolographicFaceTrackerMain::Update() {
	...
	m_timer.Tick([&] {
		...
		if(m_trackingFaces) {
			m_quadRenderer->Update(pointerPose, float3{ 0.15f, 0.00f, -2.0f }, m_timer);
		}
	...
	return m_deviceResources->UseHolographicCameraResources<bool>(
		...
		for(auto cameraPose : prediction->CameraPoses) {
			...
			if(cameraActive) {
				...
				if(m_trackingFaces) {
					if(showing_all_information) {
						m_textRenderer->RenderTextOffscreen(L"Charlie\n<cxc1921>\nGroup 1\nRe: Django Homework");
					} else {
						m_textRenderer->RenderTextOffscreen(L"Charlie\n<cxc1921>");
					}
				...
				// Otherwise we render the status message on the quad.
					...
					m_textRenderer->RenderTextOffscreen(L"No faces found");
					m_quadRenderer->RenderRGB(m_textRenderer->GetTexture());
				}
			}
...
void HolographicFaceTracker::HolographicFaceTrackerMain::on_source_pressed(SpatialInteractionManager ^ sender, SpatialInteractionSourceEventArgs ^ args) {
	showing_all_information = !showing_all_information;
}
...
void HolographicFaceTrackerMain::OnCameraAdded(HolographicSpace^ sender, HolographicSpaceCameraAddedEventArgs^ args) {
...
void HolographicFaceTrackerMain::OnCameraRemoved(HolographicSpace^ sender, HolographicSpaceCameraRemovedEventArgs^ args) {
